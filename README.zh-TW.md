...
**運作方式**：TaskAnalyzer 檢查你的請求 → 評估複雜度（1-10）→ 路由到最具成本效益的 Claude 模型 → 你在簡單任務上不會支付過高費用，同時獲得品質結果。

**實際範例**：「修正這個錯字」使用 Haiku（快速便宜），「設計認證系統」使用 Opus（最大能力），「審查這個 PR」使用 Sonnet（平衡效能）。
