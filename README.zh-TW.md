...
> **🚧 開發中 - 尚未準備好用於生產環境 🚧**
>
> 此專案目前**不穩定**，正在進行重大變更。已知問題包括測試失敗、API 不一致和功能不完整。
>
> **請勿在生產環境中使用。** 請稍後再查看穩定版本。
>
> 如需更新，請關注此存儲庫或查看 [Issues](https://github.com/PCIRCLE-AI/claude-code-buddy/issues) 頁面。
...
### 3. 💰 **智能模型選擇**
根據複雜度路由任務到正確的 Claude 模型（Haiku/Sonnet/Opus），節省約 40% 的 token 成本，同時不犧牲品質。
...
CCB 分析任務複雜度並路由到最佳的 Claude 模型，節省約 40% 的 token 成本：

- **Haiku**（簡單/便宜）- 快速任務如程式碼格式化、簡單 bug 修復
- **Sonnet**（平衡）- 標準開發工作、程式碼審查
- **Opus**（複雜/高品質）- 架構設計、複雜除錯

**運作方式**：TaskAnalyzer 檢查你的請求 → 評估複雜度 → 路由到最具成本效益的模型 → 你在簡單任務上不會支付過高費用，同時獲得品質結果。

**實際範例**：「修正這個錯字」使用 Haiku（$0.001），「設計認證系統」使用 Opus（$0.015），「審查這個 PR」使用 Sonnet（$0.003）。
